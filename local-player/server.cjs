...
=========================================================
 ...
 VERSION: 1.8.13 strict search, adult live category, recovery and analytics
 File: server.cjs
=========================================================
*/
...
const ADULT_LIVE_CATEGORY_ID = "__stb_adult_live__";
const ADULT_TERMS = /\b(?:adult|xxx|porn|erotic|sex)\b|18\s*(?:\+|plus)/i;
...
async function rebuildCatalog() {
  ...
  const adultGenreIds = new Set(
    providerCategories.filter((category) => category.locked).map((category) => category.id)
  );
  ...
  const channels = rawChannels
    ...
    .map((channel) => {
      ...
      const sourceGenreId = String(channel.tv_genre_id ?? "0");
      const channelGenre = String(
        channel.tv_genre_name || channel.genre_name || channel.genre ||
        channel.category_name || channel.tv_genre || ""
      );
      const adultLocked = isAdult(channel.name) || isAdult(channelGenre) || adultGenreIds.has(sourceGenreId);

      return {
        id,
        name: String(channel.name).trim(),
        number: Number.isFinite(number) ? number : null,
        genreId: adultLocked ? ADULT_LIVE_CATEGORY_ID : sourceGenreId,
  ...
  const adultChannels = channels.filter((channel) => channel.adultLocked);
  const categories = providerCategories.filter((category) => !adultGenreIds.has(category.id));
  if (adultChannels.length) {
    categories.push({
      id: ADULT_LIVE_CATEGORY_ID,
      title: "18+ Channels",
      locked: true,
      adultLocked: true,
    });
