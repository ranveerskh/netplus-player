...
const DIAGNOSTIC_PATH = path.join(
  ...
  "netplus-diagnostics-v1.5.0.json"
);
...
function normalizeMediaItem(row, kind = "vod") {
  ...
  const seriesFlag = String(row?.series ?? "").trim().toLowerCase();
...
async function getCombinedVodDetail(categoryId, itemId) {
  ...
  const isSeries = Boolean(item.isSeries || item.kind === "series" || seasons.length || hasSeriesDetails(info));
...
async function getCombinedVodEpisodes(categoryId, itemId, seasonNumber) {
  ...
  const info = await getVodInfo(item);
...
async function getCombinedVodEpisodeStream(categoryId, itemId, seasonNumber, episodeId) {
  const item = await findVodItem(categoryId, itemId);
  if (!item) throw new PlayerError("Series is no longer available. Refresh and try again.", 404);
  ...
  const command = episode.cmd || firstNestedValue(episode.raw, ["cmd", "command", "playback_cmd", "url", "stream_url"]);
  if (!command) {
...
function seriesInfoCandidates(info, depth = 0, seen = new Set()) {
  ...
  const wrapperKeys = ["data", "js", "info", "movie", "series", "vod", "result", "details"];
...
async function getSeriesInfo(seriesId) {
  ...
  */
  const hasUsefulInfo = hasSeriesDetails(js);
...
function downloadDiagnosticReport(res) {
  ...
  res.writeHead(200, {
    "Content-Type": "application/json; charset=utf-8",
    "Content-Length": Buffer.byteLength(body),
    "Content-Disposition": "attachment; filename=netplus-diagnostics-v1.5.0.json",
