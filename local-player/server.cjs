...
async function searchVodCatalog(query) {
  ...
  if (needle.length < 2) return { query: needle, items: [], total: 0 };
  ...
  for (const category of categories) {
    for (let page = 0; page <= 100; page += 1) {
      const result = await getVodItems(category.id, page);
      ...
      if (!result.items?.length || (result.total > 0 && (page + 1) * result.items.length >= result.total)) break;
    }
  }
