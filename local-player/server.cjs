...
const DIAGNOSTIC_PATH = path.join(
  ...
  "netplus-diagnostics-v1.6.4.json"
);
...
async function getVodItems(categoryId, page = 0, searchTerm = "") {
  /* The provider ignores its search parameter. Keep paging independent of
     search so the local index can reuse every fetched page. */
  const safePage = Math.max(0, Math.min(Number(page) || 0, 1000));
    ...
    try {
      ...
      while (!vodSearchState.complete) {
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
          try {
            result = await getVodItems(allCategory.id, page);
            break;
          } catch (error) {
            lastError = error;
            if (!/\b429\b/.test(String(error.message || "")) || attempt === 3) break;
            await new Promise((resolve) => setTimeout(resolve, 2500 * (attempt + 1)));
          }
        }

        if (!result) throw lastError || new Error("Could not read the VOD catalogue.");
        ...
        if (!pageSize || page >= Math.min(1000, maxPages - 1)) {
          vodSearchState.complete = true;
        }

        /* Yield between requests so a search index never blocks playback. */
        if (!vodSearchState.complete) await new Promise((resolve) => setTimeout(resolve, 80));
      }
    } catch (error) {
...
async function searchVodCatalog(query) {
  ...
  /* Search only the provider's All catalogue. The previous implementation
     scanned every overlapping category and sent an ignored search= value on
     every request, which caused slow searches and 429 responses. */
...
function downloadDiagnosticReport(res) {
  ...
  res.writeHead(200, {
    "Content-Type": "application/json; charset=utf-8",
    "Content-Length": Buffer.byteLength(body),
    "Content-Disposition": "attachment; filename=netplus-diagnostics-v1.6.4.json",
