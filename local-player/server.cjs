...
const DIAGNOSTIC_PATH = path.join(
  ...
  "netplus-diagnostics-v1.6.3.json"
);
...
async function getVodItems(categoryId, page = 0, searchTerm = "") {
  ...
  const safeSearch = String(searchTerm || "").trim().slice(0, 80).toLowerCase();
...
async function searchVodCatalog(query) {
  ...
  if (needle.length < 3) return { query: needle, items: [], total: 0 };

  const categories = await getVodCategories();
  const items = [];
  const seen = new Set();
  const matches = (item) => [item.title, item.description, item.year, item.rating]
    .filter(Boolean).join(" ").toLowerCase().includes(needle);

  for (const category of categories) {
    for (let page = 0; page <= 100; page += 1) {
      const result = await getVodItems(category.id, page, needle);
      for (const item of result.items || []) {
        const key = `${category.id}:${item.id}`;
        if (seen.has(key) || !matches(item)) continue;
        seen.add(key);
        items.push({
          ...item,
      ...
      if (!result.items?.length || result.items.length < 14) break;
    }
  }

  return { query: needle, items, total: items.length };
}
...
function downloadDiagnosticReport(res) {
  ...
  res.writeHead(200, {
    "Content-Type": "application/json; charset=utf-8",
    "Content-Length": Buffer.byteLength(body),
    "Content-Disposition": "attachment; filename=netplus-diagnostics-v1.6.3.json",
