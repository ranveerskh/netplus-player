const state = { catalog: null, category: "all", query: "", selected: null, hls: null, mode: "live", parentalUnlocked: false, vod: { categories: [], items: new Map(), query: "", selected: null } };
...
function renderCategories() { const categories = [{ id: "all", title: "All channels" }, ...state.catalog.categories]; elements.groupCount.textContent = `${state.catalog.categories.length} groups`; elements.categories.replaceChildren(...categories.map((category) => { const button = document.createElement("button"); button.type = "button"; button.className = `category-button${state.category === category.id ? " active" : ""}`; button.innerHTML = `<span></span><em>›</em>`; button.querySelector("span").textContent = category.locked && !state.parentalUnlocked ? "🔒 Protected content" : category.title; button.addEventListener("click", async () => { if (category.locked && !(await unlockParental())) return; state.category = category.id; renderCategories(); renderChannels(); }); return button; })); }
function filteredChannels() { const query = state.query.trim().toLowerCase(); return state.catalog.channels.filter((channel) => (state.category === "all" || channel.genreId === state.category) && (!query || channel.name.toLowerCase().includes(query))); }
function renderChannels() { const filtered = filteredChannels(); elements.channelCount.textContent = `${filtered.length.toLocaleString()} channels`; const rows = filtered.slice(0, 300).map((channel) => { const button = document.createElement("button"); button.type = "button"; button.className = `channel-button${state.selected?.id === channel.id ? " active" : ""}`; button.innerHTML = `<span class="channel-icon"></span><span class="channel-copy"><strong></strong><small></small></span><span class="channel-play">▶</span>`; button.querySelector(".channel-icon").textContent = initials(channel.name); button.querySelector("strong").textContent = channel.name; button.querySelector("small").textContent = `${channel.number ? `Channel ${channel.number}` : "Live channel"}${channel.hd ? " · HD" : ""}`; button.addEventListener("click", () => playLive(channel)); return button; }); if (filtered.length > 300) { const note = document.createElement("p"); note.className = "list-note"; note.textContent = "Showing first 300 results. Search to narrow the list."; rows.push(note); } elements.channels.replaceChildren(...rows); }
...
elements.settingsButton.addEventListener("click", showSetup); elements.vodClose.addEventListener("click", () => { elements.vodModal.hidden = true; }); elements.vodPlayButton.addEventListener("click", () => { elements.vodModal.hidden = true; beginPlayback(state.vod.selected); setMode("live"); }); document.querySelectorAll(".mode-button").forEach((button) => button.addEventListener("click", () => setMode(button.dataset.mode)));
