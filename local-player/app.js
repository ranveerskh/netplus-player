...
=========================================================
 NetPlus IPTV Player
 VERSION: 1.6.4 Reliable catalogue/search/input controls
 File: app.js
=========================================================
*/
...
/* v1.6.4: browser/HLS events are sent without any portal URL,
...
async function openContentBrowser() {
  ...
  if (allCategory && state.vod.categoryId !== allCategory.id) await selectVodCategory(allCategory.id);
}
...
for (const input of [elements.mac, elements.parentalPin, elements.unlockPin, elements.newParentalPin]) {
...
function filterVodCollection(collection) {
  ...
  return typeFiltered.filter((item) =>
    [item.title, item.description, item.year, item.rating, item.director, item.actors, item.path, item.categoryTitle]
      .filter(Boolean).join(" ").toLowerCase().includes(query)
  );
}

function filteredVodItems() {
  return filterVodCollection(state.vod.searchResults || state.vod.items);
}
...
async function loadNextVodPage(reset = false) {
  ...
  try {
    ...
    if (incoming.length === 0) {
      ...
      elements.vodEndMessage.hidden = true;
      setTimeout(() => {
        if (token === state.vod.loadToken) loadNextVodPage(false);
      }, 180);
      return;
    } else if (added === 0) {
      state.vod.ended = true;
...
async function openVodModal(item) {
  ...
  if ((item.categoryLocked || itemCategory?.locked) && !state.parentalUnlocked) {
    requestParentalUnlock(() => openVodModal(item));
    return;
  }
...
async function playVod(item, resumeFrom = 0, qualityId = "", isRenewal = false) {
  ...
  if (!document.fullscreenElement && !isRenewal) toggleFullscreen(elements.vodPlayerContainer);
...
async function openQualityModal({ kind, item, series, season, episode, resumeFrom = 0 }) {
  ...
  try {
    ...
    const buttons = options.map((option, index) => {
      ...
      button.addEventListener("click", () => {
        closeQualityModal();
        /* VOD playback is an in-app full-screen overlay, with the native
           Full button still available for true OS/browser full screen. */
...
function qualityDisplayLabel(option, index = 0) {
  ...
  if (!quality && (/^\d+$/.test(raw) || !raw || /default/.test(lower))) {
    return index === 0 ? "Recommended · Full HD 1080p" : `Quality option ${index + 1}`;
  }
  if (!quality) return raw.length > 70 ? `Quality option ${index + 1}` : raw;
  return language && !/^\d+$/.test(language) ? `${language} · ${quality}` : quality;
}
...
function openDashboardMedia(item) {
  setVodFilter(item.mediaType === "series" || item.kind === "series" ? "series" : "movie");
  if (item.categoryId) openVodModal(item);
}
...
function makeDashboardMediaCard(item, { resume = false, removable = false } = {}) {
  ...
  const meta = document.createElement("small");
  meta.textContent = resume
    ? `${item.mediaType === "series" ? "Series" : "Movie"} · Resume ${formatTime(item.time)}`
    : `${item.kind === "series" || item.mediaType === "series" ? "Series" : "Movie"}${item.year ? ` · ${item.year}` : ""}`;
...
function renderDashboard() {
  ...
  const entries = Object.entries(state.watchHistory)
    ...
    .map(([id, time]) => ({ ...(state.watchMeta[id] || { id, title: "Saved title" }), time: Number(time) }))
  ...
  const available = state.vod.items.filter((item) => item?.id && !state.watchHistory[item.id]);
  const stableShelf = (key, items) => {
    const saved = Array.isArray(state.homePicks?.[key]) ? state.homePicks[key] : [];
    const sorted = [...items];
    const byId = new Map(sorted.map((item) => [String(item.id), item]));
    const ordered = saved.map((id) => byId.get(String(id))).filter(Boolean);
    const fresh = sorted.filter((item) => !saved.includes(String(item.id)));
    const result = [...ordered, ...fresh].slice(0, 8);
    state.homePicks[key] = result.map((item) => String(item.id));
    localStorage.setItem("netplusHomePicks", JSON.stringify(state.homePicks));
    return result;
  };
  const recommended = stableShelf("recommended", [...available].sort((a, b) => String(a.title || "").localeCompare(String(b.title || ""), undefined, { numeric: true, sensitivity: "base" }) || String(a.id).localeCompare(String(b.id))));
  const popular = stableShelf("popular", [...state.vod.items]
  ...
  if (elements.popularGrid) elements.popularGrid.replaceChildren(
    ...(popular.length ? popular.map((item) => makeDashboardMediaCard(item)) : [emptyShelf("Your provider's popular titles will appear here after the catalogue loads.")])
  );
}
...
async function searchAllVod(query, poll = false) {
  ...
  if (!poll) {
    state.vod.searching = true;
    renderVodGrid();
  }

  try {
    const result = await request(`/api/vod/search?q=${encodeURIComponent(text)}`);
    if (token !== state.vod.searchToken || state.vod.query.trim() !== text) return;

    state.vod.searchResults = Array.isArray(result.items) ? result.items : [];
    state.vod.searching = false;
    state.vod.searchIndexing = !result.complete;
    state.vod.searchIndexedItems = Number(result.indexedItems) || 0;
    state.vod.searchTotalItems = Number(result.totalItems) || 0;
    renderVodGrid();

    if (!result.complete && !result.error) {
      vodSearchPollTimer = setTimeout(() => searchAllVod(text, true), 1400);
    } else if (result.error) {
      showNotice(`Search index paused: ${result.error}`);
    }
  } catch (error) {
    ...
    renderVodGrid();
    showNotice(`Search failed: ${error.message}`);
  }
}
...
async function refreshContent(manual = true) {
...
elements.shareButton?.addEventListener("click", async () => {
  try {
    await navigator.share({ title: "NetPlus IPTV Player", text: "NetPlus IPTV Player" });
  } catch {}
});
...
elements.downloadDiagnosticButton?.addEventListener("click", () => {
  ...
  link.download = "netplus-diagnostics-v1.6.4.json";
  document.body.append(link);
  link.click();
  link.remove();

  elements.diagnosticNotice.textContent = "Report download started. Send the netplus-diagnostics-v1.6.4.json file here.";
  elements.diagnosticNotice.style.color = "#35dbc5";
  elements.diagnosticNotice.hidden = false;
});
...
elements.resetPortalButton.addEventListener("click", () => {
  ...
  /* A full existing MAC plus the old maxlength handler made replacement
     keystrokes disappear. Reconfigure starts with an empty editable field. */
  elements.mac.value = "";
  elements.parentalPin.value = "";
  showSetup();
  requestAnimationFrame(() => elements.mac?.focus());
});
