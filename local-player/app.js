...
=========================================================
 NetPlus IPTV Player
 VERSION: 1.6.3 UI refresh + Fresh VOD create_link Fix
 File: app.js
=========================================================
*/
...
/* v1.6.3: browser/HLS events are sent without any portal URL,
...
if (elements.mac) {
  ...
  elements.mac.addEventListener("input", () => {
    /* Keep the field editable while typing. Format on blur/submit so
       replacing an existing MAC does not constantly move the caret. */
    elements.mac.value = elements.mac.value.replace(/[^0-9a-fA-F:]/g, "").slice(0, 17).toUpperCase();
  });
...
async function loadNextVodPage(reset = false) {
  ...
  try {
    ...
    /* This portal sometimes ignores p=1 and returns page zero again. Skip
       forward a few times before declaring the category finished. */
...
let vodSearchTimer;
async function searchAllVod(query) {
...
elements.downloadDiagnosticButton?.addEventListener("click", () => {
  ...
  link.download = "netplus-diagnostics-v1.6.3.json";
  document.body.append(link);
  link.click();
  link.remove();

  elements.diagnosticNotice.textContent = "Report download started. Send the netplus-diagnostics-v1.6.3.json file here.";
  elements.diagnosticNotice.style.color = "#35dbc5";
  elements.diagnosticNotice.hidden = false;
});
