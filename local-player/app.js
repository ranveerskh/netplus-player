...
/* v1.5.0: browser/HLS events are sent without any portal URL,
...
function renderVodGrid() {
  ...
  const cards = items.map((item) => {
    ...
    const meta = document.createElement("small");
    meta.textContent =
      [item.kind === "series" ? "Series" : "Movie", item.year, item.rating && `★ ${item.rating}`].filter(Boolean).join(" · ");

    const badge = document.createElement("span");
    badge.className = `vod-type-badge ${item.kind === "series" ? "is-series" : "is-movie"}`;
    badge.textContent = item.kind === "series" ? "SERIES" : "MOVIE";
...
async function loadCombinedSeriesEpisodes(season) {
  ...
  try {
    ...
    const list = episodes.map((episode, index) => {
      ...
      button.addEventListener("click", () => playCombinedEpisode(series, season, episode));
      return button;
    });
    if (!list.length) {
...
async function playCombinedEpisode(series, season, episode) {
  ...
  try {
    const payload = await request("/api/vod/episode/play", {
      ...
      body: JSON.stringify({ categoryId: series.categoryId, itemId: series.id, season: Number(season), episodeId: String(episode.id) }),
    });
...
elements.downloadDiagnosticButton?.addEventListener("click", () => {
  ...
  link.download = "netplus-diagnostics-v1.5.0.json";
  document.body.append(link);
  link.click();
  link.remove();

  elements.diagnosticNotice.textContent = "Report download started. Send the netplus-diagnostics-v1.5.0.json file here.";
  elements.diagnosticNotice.style.color = "#35dbc5";
  elements.diagnosticNotice.hidden = false;
});
