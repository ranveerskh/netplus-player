...
=========================================================
 NetPlus IPTV Player
 VERSION: 1.5.2 Mixed VOD + Live Stability
 File: app.js
=========================================================
*/
...
/* v1.5.2: browser/HLS events are sent without any portal URL,
    ...
    */
    ...
    if (!data.fatal && isMediaSourceReset) {
      if (now - lastResetRecoveryAt > 8_000) {
        ...
        try { hls.recoverMediaError(); } catch { freshLink(); }
      }
      return;
    }
...
elements.downloadDiagnosticButton?.addEventListener("click", () => {
  ...
  link.download = "netplus-diagnostics-v1.5.2.json";
  document.body.append(link);
  link.click();
  link.remove();

  elements.diagnosticNotice.textContent = "Report download started. Send the netplus-diagnostics-v1.5.2.json file here.";
  elements.diagnosticNotice.style.color = "#35dbc5";
  elements.diagnosticNotice.hidden = false;
});
