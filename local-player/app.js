...
=========================================================
 ...
 VERSION: 1.7.5 Premium Home + stable hero, catalogue, episode and theme fixes
 File: app.js
=========================================================
*/
...
function searchLocalVodIndex(query) {
  ...
  return state.vod.localIndex.filter((item) => {
    ...
    return [item.title, item.description, item.year, item.rating, item.director, item.actors, item.genre, item.language, item.audioLanguage, item.path, item.categoryTitle]
...
async function activatePortal(id) {
  try { const result = await request("/api/portals/activate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id }) }); state.portals = result.portals || state.portals; state.activePortalId = result.activePortalId || id; renderPortalList(); await refreshContent(false); setSettingsNotice("Active portal changed. Content refreshed."); }
...
function filterVodCollection(collection) {
  ...
  return typeFiltered.filter((item) =>
    [item.title, item.description, item.year, item.rating, item.director, item.actors, item.genre, item.language, item.audioLanguage, item.path, item.categoryTitle]
...
function qualityDisplayLabel(option, index = 0) {
  ...
  if (!quality && (/^\d+$/.test(raw) || !raw || /default/.test(lower))) {
    return index === 0 ? "Play · Full HD 1080p" : `Play · Alternative quality ${index + 1}`;
  }
  if (!quality) return raw.length > 70 ? `Play · Alternative quality ${index + 1}` : `Play · ${raw}`;
  return language && !/^\d+$/.test(language) ? `Play · ${language} · ${quality}` : `Play · ${quality}`;
}
...
elements.portalEditorForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    renderPortalList(); elements.portalEditorModal.hidden = true; setSettingsNotice("Portal saved. Content refreshed."); await refreshContent(false);
...
elements.supportButton?.addEventListener("click", () => setSettingsNotice("Contact your STB PLAY provider for portal or MAC support."));
...
elements.downloadDiagnosticButton?.addEventListener("click", () => {
  ...
  link.download = "netplus-diagnostics-v1.7.5.json";
  document.body.append(link);
  link.click();
  link.remove();

  elements.diagnosticNotice.textContent = "Report download started. Send the netplus-diagnostics-v1.7.5.json file here.";
  elements.diagnosticNotice.style.color = "#35dbc5";
  elements.diagnosticNotice.hidden = false;
});
