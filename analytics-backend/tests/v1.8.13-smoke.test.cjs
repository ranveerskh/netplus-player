  ...
  const server = http.createServer((req, res) => {
    ...
    if (action === "get_all_channels") {
      const channels = [
        state.staleChannel
          ? null
          : { id: "100", name: "News One", cmd: "http://127.0.0.1/stream/news", tv_genre_id: "1", number: 1 },
...
test("v1.8.13 release markers and recovery/search boundaries are present", () => {
  ...
  assert.equal(packageJson.version, "1.8.13");
  assert.equal(updateJson.version, "1.8.13");
  assert.match(updateJson.downloadUrl, /v1\.8\.13\/Netplus-IPTV-Player-Setup-1\.8\.13\.exe$/);
  assert.match(app, /function strictTitleSearchMatch/);
  assert.match(app, /!query \|\| strictTitleSearchMatch\(\{ title: channel\.name \}, query\)/);
  assert.match(server, /ADULT_LIVE_CATEGORY_ID = "__stb_adult_live__"/);
  assert.match(server, /title: "18\+ Channels"/);
...
test("live catalogue adds the PIN category and recovers 401/stale playback", async (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "stb-play-live-v1.8.13-"));
  ...
  const adultCategory = catalog.categories.find((category) => category.id === "__stb_adult_live__");
  assert.deepEqual(
    { title: adultCategory?.title, locked: adultCategory?.locked },
    { title: "18+ Channels", locked: true }
  );
  assert.equal(catalog.channels.find((channel) => channel.id === "200")?.genreId, "__stb_adult_live__");
  assert.equal(catalog.categories.some((category) => category.id === "2"), false);
...
test("local player queues and delivers an analytics event to the backend", async (t) => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "stb-play-v1.8.13-"));
  ...
  assert.equal(store.events[0].version, "1.8.13");
  ...
  const invalid = await fetch(`http://127.0.0.1:${playerPort}/api/analytics/event`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ installationId: "bad", name: "app_opened", version: "1.8.13", platform: "linux" }),
  });
